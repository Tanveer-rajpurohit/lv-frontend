...
export default function NewProjectModal({ isOpen, onClose }: NewProjectModalProps) {
  ...
  const [selectedDocument, setSelectedDocument] = useState('research_paper');
  ...
  const handleCreate = async () => {
    ...
    try {
      ...
      setProjectName('');
      setSelectedDocument('research_paper');
      setAuthorNames([]);
      setInstituteName('');
      setSupervisorFaculties([]);
      setAuthorInput('');
      setSupervisorInput('');
      onClose();
      ...
      if (newProject?.id) {
        router.push(`/writing-section/${newProject.id}`);
      }
    } catch (error) {
